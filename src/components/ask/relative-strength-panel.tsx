...
export function RelativeStrengthPanel({ ticker, timeframeRange }: RelativeStrengthPanelProps) {
  ...
  const drawChart = useCallback(() => {
    ...
    ctx.fillStyle = C.textPrimary; ctx.fillText(ticker, W - PAD.right + 4, yPos(last.tickerPct) + 3);
    ctx.fillStyle = 'rgba(0,220,130,0.6)'; ctx.fillText('SPY', W - PAD.right + 4, yPos(last.spyPct) + 3);
    ctx.fillStyle = 'rgba(167,139,250,0.6)'; ctx.fillText('QQQ', W - PAD.right + 4, yPos(last.qqqPct) + 3);

    // X-axis
    ctx.fillStyle = C.textMuted; ctx.font = `500 9px ${FONT_MONO}`; ctx.textAlign = 'center';
    const step = Math.max(Math.floor(N / 8), 1);
    for (let i = 0; i < N; i += step) {
      if (filteredData[i]?.time) ctx.fillText(filteredData[i].time, xPos(i), H - PAD.bottom + 14);
    }
